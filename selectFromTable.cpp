...
int selectFromTable(BaseClass *ptrbc, int mycase) {
    ...
    if (ptrbc->debugFlags.debug2 ) std::cout << theCases[mycase] <<__TIMESTAMP__ << "\nThe mother of all select statements looks like:\n" << motherOfAllSelectStatements[mycase] << std::endl;
//============
...
//===============
    ...
    if (!ptrbc->debugFlags.tableCreated) {  //Have we created the table?
        try {
            ptrbc->rc[SELECTFROMTABLE] = dropTheTable(ptrbc, dropTable);   //First drop the pre-existing table to get a fresh start.
        } catch (const std::exception &e) {
            std::cerr << "Failed to drop the table, tbl_bucketized_by_tempf_ranges. EXITING " << cases[mycase] << std::endl;
            exit(1);
        }
        try {
            if(createTheTable(ptrbc, createTable, true)) std::cerr << "Failed to create the table, tbl_bucketized_by_tempf_ranges. EXITING " << cases[mycase] << std::endl; //Go create table tbl_bucketized_by_tempf_ranges
        } catch (const std::exception &e) {
            std::cerr << "Failed to create the table, tbl_bucketized_by_tempf_ranges. EXITING " << cases[mycase] << std::endl;
            exit(2);
        }
...
//     MOVED TO myPrototypes   enum colnames {_currentavtempf,  _previousavtempf,  _davtempf,  _currentavtempk,  _previousavtempk,   _previousdate,   _currentdate, _dday,  _previousm1kwh, _currentm1kwh, _m1Usage, _previousm2kwh,  _currentm2kwh, _m2Usage, _AvgDailyUsage};
    ...
    if (PQresultStatus(ptrbc->res) != PGRES_COMMAND_OK) {
        std::cerr << "cases[mycase]" <<"Failed to execute the SQL statement: " << begin_Transaction << ". Error cause:\n" << PQerrorMessage(ptrbc->conn) << std::endl;
        ptrbc->rc[SELECTFROMTABLE] = BEGINTRANSACTIONFAILED;
    }
    //    PQclear(ptrbc->res);

//    rc = executeSQL(ptrC, ptrbc->res, ptrBigSQL);
    ptrbc->res = PQexec(ptrbc->conn,  motherOfAllSelectStatements[mycase]);
    if (PQresultStatus(ptrbc->res) != PGRES_COMMAND_OK) {
        std::cerr << "Failed to execute the SQL statement: " << motherOfAllSelectStatements[mycase] << ". Error cause:\n" <<
        PQerrorMessage(ptrbc->conn) << cases[mycase] << std::endl;
...
//    rc = fetchAllTransaction(ptrC,ptrbc->res, fetchALLTransaction);
    ...
    if (PQresultStatus(ptrbc->res) != PGRES_TUPLES_OK) {
        ...
        cerr << "Line " << __LINE__ << "Of file " << __FILE__ << "Failed when attempting to execute this sql:\n" << motherOfAllSelectStatements[mycase] << "\nWe got a return status of: " << PQresultStatus(ptrbc->res) << cases[mycase] << endl;
...
#define NEITHERFIRSTORLAST 2 //This means we will not include the first elemet or the last element because they are misleading and erroneous: \
      The first element lacks the previous day's reading;  \
      likely, the last element does not have a full-day's worth of readings.
    ...
    if (ptrbc->debugFlags.debug2 ) std::cout  <<headerRow[0] << "\t" << theCases[mycase] << std::endl;
    if (ptrbc->debugFlags.debug2 ) std::cout  <<headerRow[1] << "\t" << theCases[mycase] << std::endl;
        ...
        try {
            ...
            switch (ptrbc->debugFlags.mycase) {
                ...
                case M2KWH:
                    ...
                    break;
                default:
                    cerr << "Line " << __LINE__ << "File " << __FILE__ << "It seems like we picked up an undefined value for ptrbc->debugFlags.mycase: " << ptrbc->debugFlags.mycase << "\t We're terminating." << cases[mycase] << endl;
                    exit (BADMYCASEVALUE);
            }
...
//            strcpy(ptrbc->copyBuffer+ptrbc->bufferOffset, ", ");
            ...
            strcpy(ptrbc->copyBuffer + ptrbc->bufferOffset, ptrbc->eu);
...
//        strcpy(ptrbc->copyBuffer+ptrbc->bufferOffset, "\n");
            ...
            *(ptrbc->copyBuffer + ptrbc->bufferOffset) = '\0'; //Terminate our growing string. Do we really have to do this since we did a massive memset just after allocating this dynamic instruction?
            ...
            std::cerr << "On row: " << ptrbc->grc << ", we encountered some kind of conversion error, like trying to convert a null to an int or a float. [["<< e.what() << "]] Skipping this row, however processing continues." << cases[mycase] << std::endl;
            ++ptrbc->brc; //increment the bad row count.
        }
    ...
    if (!ptrbc->debugFlags.tempTableCreated) {  //Have we created the temporary table?
        ...
        ptrbc->res = PQexec(ptrbc->conn, drop_tbl_temp_eu);
        if (PQresultStatus(ptrbc->res) != PGRES_COMMAND_OK ) {
            cerr << drop_tbl_temp_eu << " command failed " << cases[mycase] << endl;
            exit_nicely(ptrbc, DROPTABLEFAILED, __LINE__);
        }
        PQclear(ptrbc->res);
        ptrbc->res = PQexec(ptrbc->conn, create_tbl_temp_eu);
        if (PQresultStatus(ptrbc->res) != PGRES_COMMAND_OK ) {
            cerr << create_tbl_temp_eu<< " command failed " << cases[mycase] << endl;
...
// BEGINNING OF COPY OPERATION
    ...
    ptrbc->resultsPQputCopyEnd = PQputCopyEnd(ptrbc->conn, errmsg);
//    int whichIsIt = PQresultStatus(ptrbc->res);
    if (ptrbc->debugFlags.debug2 )  cout << " resultsPQputCopyData: " << ptrbc->resultsPQputCopyData << "; resultsPQputCopyEnd: " << ptrbc->resultsPQputCopyEnd << cases[mycase] << endl;
    
    if (ptrbc->errmsg) {
        cerr << "COPY operation failed with error message of: " << errmsg << cases[mycase] << endl;
    } else {
        if (ptrbc->debugFlags.debug2 ) cout << "COPY operation seems to have worked!" << cases[mycase] << endl;
...
//END OF COPY OPERATION
    ...
    ptrbc->rc[SELECTFROMTABLE] = executeSQL(ptrbc, theMotherOfAllUnionSelectStatements);   //
    if (ptrbc->rc[SELECTFROMTABLE]) exit_nicely(ptrbc, ptrbc->rc[SELECTFROMTABLE], __LINE__);
    ...
    if (ptrbc->rc[SELECTFROMTABLE]) { //Note that PGRES_TUPLES_OK happens when PQresultStatus returns a 2.
        ...
        ptrbc->numberOfReturnedRows = PQntuples(ptrbc->res);
        ptrbc->numberOfReturnedColumns = PQnfields(ptrbc->res);
        if (ptrbc->debugFlags.debug2 ) cout << "TheMotherOfAllUnionSelectStatements returned " << ptrbc->numberOfReturnedRows << " rows and " <<    ptrbc->numberOfReturnedColumns << " columns." << cases[mycase] <<  endl;
    }

    if (ptrbc->debugFlags.debug2 ) cout << "Now bucketize the data. Each bucket has a temperature range of " << BUCKETSIZE << "ºF " << " and we will get the average energy usage, in kwh, for that temperature range buffer along with the energy usage standard deviation. " << cases[mycase] << endl;
    ...
    const char *headerEU[2] = {"  avgtemp | avg eu |      stddev       |  min  |  max  | count\n", \
                                 "----------+--------+-------------------+-------+-------+-------\n"};
    if (ptrbc->debugFlags.debug2 ) cout << *(headerEU +0) << *(headerEU + 1) << cases[mycase] << std::endl; //Output the headers.//
    ...
    while ( ptrbc->buckettemp < ptrbc->maxBucketTemp && ptrbc->loopCounter < ptrbc->numberOfReturnedRows ) {  //Beginning of while statement.
        ...
        try {
            if (ptrbc->debugFlags.debug2 ) {
                ...
                std::cout << a_ff->alignCenter(5,  PQgetvalue(ptrbc->res, ptrbc->loopCounter, _countu) ) << "|" <<  cases[mycase] << endl;
            }
            if( (ptrbc->thisCount =atoi( PQgetvalue(ptrbc->res, ptrbc->loopCounter, _countu) ) ) > 1 ) {
                ...
                if (ptrbc->rc[SELECTFROMTABLE]) {
                    cerr << "Operation to INSERT INTO tbl_bucketized_by_tempf_ranges  F A I L E D!" << cases[mycase] << std::endl;
                    exit_nicely(ptrbc, ptrbc->rc[SELECTFROMTABLE], __LINE__);
                }
            ...
            std::cerr << __FILE__ "2. bad values try/catch detected errors for entry number " << ptrbc->debugFlags.countOfBadEntries + ptrbc->countOfGoodEntries << cases[mycase] << std::endl;
...
// =================================================================
    ...
    if (ptrbc->worthyOfPolynomialFitProcessing > POLYNOMIALDEGREE) {
        ...
        try {  //Attempt to read back the data we just inserted into table tbl_bucketized_by_tempf_ranges
            ...
            if (ptrbc->rc[SELECTFROMTABLE]) exit_nicely(ptrbc, ptrbc->rc[SELECTFROMTABLE], __LINE__);
            ptrbc->rc[SELECTFROMTABLE] = executeSQL(ptrbc, readBackTable);  //We might want to order by temp.
            if (ptrbc->rc[SELECTFROMTABLE]) exit_nicely(ptrbc, ptrbc->rc[SELECTFROMTABLE], __LINE__);
            ptrbc->rc[SELECTFROMTABLE] = fetchAllTransaction(ptrbc, fetchALLTransaction);

            if (ptrbc->rc[SELECTFROMTABLE]) {
                if (ptrbc->debugFlags.debug2 ) cout << "FETCH ALL for SELECT statement\n" << readBackTable << "\nfailed with error: " << PQerrorMessage(ptrbc->conn) << cases[mycase] << endl;
            } else {
                ptrbc->numberOfReturnedRows = PQntuples(ptrbc->res);
                ptrbc->numberOfReturnedColumns = PQnfields(ptrbc->res);
            }
            ...
            if (ptrbc->numberOfReturnedRows) {
                if (ptrbc->numberOfReturnedRows != ptrbc->worthyOfPolynomialFitProcessing) {
                    std::cerr << "For some unknown reason, the numberOfReturnedRows: " << ptrbc->numberOfReturnedRows << " AND values worthyOfPolynomialFitProcessing: " << ptrbc->worthyOfPolynomialFitProcessing << " have different values. " << cases[mycase] << std::endl;
                }
                ...
                for  (ptrbc->loopCounter = 0, ptrbc->rowBeingProcessed = 0; ptrbc->loopCounter < ptrbc->numberOfReturnedRows; ++ptrbc->loopCounter, ++ptrbc->rowBeingProcessed)
...
// NO LONGER NEEDED                    ++rowBeingProcessed;
                ...
                ptrbc->rc[SELECTFROMTABLE] = endTransaction(ptrbc, end_Transaction);  //Get another `WARNING:  there is no transaction in progress`
                
                ptrbc->rc[SELECTFROMTABLE] = smf->doMultiFit( ); //Actually go out and do the gsl_multifit
                ptrbc->rc[SELECTFROMTABLE] = smf->outputPolynomial("EnergyUsage(T) = ", cases[mycase]);
                std::cout << smf->outputCovarianceMatrix("cov = [", cases[mycase]) << "☜ χ-squared" << cases[mycase] << std::endl;
                std::cout <<smf->computeTrace(smf->covarienceMatrix->data, (int)smf->onePlusPolynomialDegree) << "☜ Trace of Covariance Matrix" << cases[mycase]  << std::endl;
                std::cout << smf->computeCorrelationBetweenIndependentAndDependentVariables(ptrbc) << "☜ correlation coefficient" << cases[mycase] << std::endl;
                ...
                std::cout << ptrbc->squareRootOfSumOfTheSquaresOfTheVarience << "☜ Square Root Of Sum Of TheSquares Of The Varience; \n" << ptrbc->averageVariencePerValidReading << "☜ Average Varience Per Valid Reading\n" << ptrbc->averageVariencePerValidReading*100 << "☜ \% Average Varience Per Valid Reading\n"<< ptrbc->grc << "☜ Number of valid meter readings (observed dependent variable)" <<  cases[mycase] << std::endl;
                /*
                ptrbc->outstring <<  ptrbc->squareRootOfSumOfTheSquaresOfTheVarience << "☜☜ Square Root Of Sum Of TheSquares Of The Varience; \n" << ptrbc->averageVariencePerValidReading << "☜ Average Varience Per Valid Reading\n" << ptrbc->averageVariencePerValidReading*100 << "☜ \% Average Varience Per Valid Reading\n"<< ptrbc->grc << "☜ Number of valid meter readings (observed dependent variable)" << cases[mycase] << std::endl; //Experiment
                std::cout << ptrbc->outstring->str() << endl;  //Experiment.
                 */
            } else {
                std::cerr << "Although the SELECT seems to have worked, we got " << ptrbc->numberOfReturnedRows << " rows back from the table which we thought we just inserted data into!" << cases[mycase] << std::endl;
            }
        } catch (const std::exception &e) {
            std::cerr << "Some how we failed to get any data back from the table which we thought we just inserted data into!" << cases[mycase] << std::endl;
        }
 
        
        //        rc = polynomialFit(POLYNOMIALDEGREE, grc, awv->temperatureVector, awv->energyUsageVector,  NULL, NULL,  &TSS, &goodnessOfFit);
    } else {
        std::cerr << "Insufficient number of VALID datapoints to do MultiFit processing for desired polynomial of degree " << POLYNOMIALDEGREE << cases[mycase] << std::endl;
    }
