...
private: //These values get set during execution time, not during compile time.
    ...
    CmdLineArgs *CLA;   //Points to the Command Line Arguments Class.
public:
...
//Variables specifict to selectFromTable
    ...
    std::stringstream *outstring; //Create a pointer to object that replaces std::cout or std::cerr with a character string.
    size_t len;
